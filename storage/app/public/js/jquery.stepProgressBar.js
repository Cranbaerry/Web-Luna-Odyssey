...
(function ($){
    ...
    StepProgressBar.prototype = {
        ...
        _calcCurrentProgressValue: function() {
            if(isNullOrUndefined(this.settings.steps) || (this.settings.steps && this.settings.steps.length <= 0)) {
                ...
                // this.progressValue = getPercentValue(this.settings.currentValue, minValue, maxValue, this.settings);

                var nextValue = this.settings.steps[0].value;
                var prevValue = this.settings.currentValue >= nextValue ? nextValue : 0;
                var count = 0;
                var equal = false;

                for (var i = 0; i < this.settings.steps.length; i++) {
                    prevValue = i == 0 ? 0 : this.settings.steps[i-1].value;

                    if ((this.settings.steps[i].value >= this.settings.currentValue)
                      || i == this.settings.steps.length - 1 && this.settings.currentValue >= this.settings.steps[i].value) {
                        nextValue = this.settings.steps[i].value;
                        break;
                    }

                    if (this.settings.currentValue == this.settings.steps[i].value)
                        equal = true;
                ...
                console.log("current: " + this.settings.currentValue);

                console.log('prev: ' + prevValue);
                console.log('next: ' +  nextValue);
                console.log('diff1: ' + (this.settings.currentValue - prevValue));
                console.log('diff2: ' + (nextValue - this.settings.currentValue));

                this.progressValue = (100 / this.settings.steps.length * (count));
                this.progressValue += equal ? 0 : ((this.settings.currentValue - prevValue) / (nextValue - prevValue) * 20);
        ...
        _calcStepsProgressValues: function() {
            if(isNullOrUndefined(this.settings.steps) || (this.settings.steps && this.settings.steps.length <= 0)) {
                ...
                for(var i = 0; i < this.settings.steps.length; i++) {
                    ...
                    step.progressValue = getPercentValue(step.value, minValue, maxValue, this.settings, i);
                }
        ...
        _buildTopLabel: function(step, index) {
            ...
            var label = step.topLabel ? step.topLabel : (!isNullOrUndefined(step.value) ? (number_format(step.value) + this.settings.unit) : '');
    ...
    //
    var getPercentValue = function(value, min, max, settings, index) {
        if(isNullOrUndefined(value))
            ...
            if(isNullOrUndefined(min) && isNullOrUndefined(max))
                ...
                if(value < min)
                    ...
                    //console.log("Index: " + (index + 1) +", Val: "+ value +", Old: " + ((value - min) / (max - min) * 100) + ", New: " + (100 / settings.steps.length * (index + 1)));
                    return (100 / settings.steps.length * (index + 1));
                }
            }
        }
    };
